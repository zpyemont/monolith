...
# Model persistence
# Note: 'model_dir' and 'zk_server' are already defined by monolith.native_training.gflags_utils
...
class FashionRankingBatchTraining(FashionRankingModelBase):
    ...
    def _try_bigquery_input(self, mode):
        ...
        try:
            ...
            if is_training:
                # Repeat dataset for multiple epochs, shuffle, and batch
                return ds.repeat().shuffle(10000).batch(512, drop_remainder=True)
            else:
                return ds.batch(512, drop_remainder=True)
    ...
    def eval_input_fn(self, mode=None):
        ...
        ds = tf.data.Dataset.from_tensor_slices(data)
        ds = ds.batch(512, drop_remainder=False)  # No shuffle, keep all samples
        return ds.map(_to_ragged).prefetch(tf.data.AUTOTUNE)
...
class FashionRankingOnlineTraining(FashionRankingModelBase):
    def input_fn(self, mode):
        ...
        def decode_json_training_sample(kafka_message):
            """Decode JSON training sample with embeddings"""
            try:
                ...
                # Extract embeddings
                text_embedding = product_data.get('text_embedding', [0.0] * TEXT_EMB_DIM)
                image_embedding = product_data.get('image_embedding', [0.0] * IMAGE_EMB_DIM)

                # Hash string IDs to int64
                ...
                return {
                    ...
                    'image_embedding': [0.0] * IMAGE_EMB_DIM,
                }
...
def main(_argv):
    ...
    if FLAGS.training_type == 'batch':
        # Train with periodic logging - loss is logged every 100 steps by TensorFlow
        # With 33k train samples / 512 batch = 65 steps per epoch
        # We train for 10000 steps total (~154 epochs through the data)
        max_steps = 10000

        logging.info("=" * 60)
        logging.info("Starting training - watch for 'loss = X.XX, step = N' messages")
        logging.info("Loss should decrease from ~30 to ~0.3 as training progresses")
        logging.info("=" * 60)

        # Train using max_steps (works in distributed mode)
        # Note: For local testing, use steps= instead of max_steps=
        estimator.train(max_steps=max_steps)

        logging.info("=" * 60)
        logging.info("TRAINING COMPLETE")
        logging.info("Check the loss values above to verify the model learned:")
        logging.info("- Initial loss should be ~30")
        logging.info("- Final loss should be ~1-2 (good) or ~5-10 (ok)")
        logging.info("=" * 60)

    else:
        # Online training - no validation, just continuous training
        estimator.train()
